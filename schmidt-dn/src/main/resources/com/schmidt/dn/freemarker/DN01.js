...
function addItem() {
			...
			var item = sampleBody.childNodes[1].cloneNode(true);
			var number = 1;
					...
					for(var tcid = 0; tcid < item.childNodes[trid].childNodes.length; tcid++) {
						if(item.childNodes[trid].childNodes[tcid].nodeName == "INPUT") {
							if (item.childNodes[trid].childNodes[tcid].getAttribute("id") != null) {
								...
								item.childNodes[trid].childNodes[tcid].setAttribute("onclick",
								item.childNodes[trid].childNodes[tcid].getAttribute("onclick").replace("[id]", tnumber)
								);
							}
						}
					}
